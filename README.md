<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
<title>AIDE ETR N1 – Ouverture de dossier étranger</title>

<style>
    body {
        font-family: Arial, sans-serif;
        background: #eef2f7;
        margin: 0;
        padding: 0;
    }

    .wrapper {
        max-width: 850px;
        margin: 40px auto;
        background: #ffffff;
        padding: 30px 35px;
        border-radius: 14px;
        box-shadow: 0 4px 15px rgba(0,0,0,0.18);
    }

    h1 {
        text-align: center;
        color: #154360;
        font-size: 30px;
        margin-bottom: 5px;
        font-weight: 800;
    }

    .subtitle {
        text-align: center;
        color: #566573;
        margin-bottom: 25px;
        font-size: 15px;
    }

    h2 {
        font-size: 18px;
        margin-top: 25px;
        color: #1b4f72;
        border-left: 5px solid #1b4f72;
        padding-left: 10px;
    }

    label {
        font-weight: 700;
        margin-top: 15px;
        display: block;
        color: #1f2d3d;
    }

    .small-help {
        font-size: 12px;
        color: #5f6a6a;
        margin-bottom: 6px;
    }

    input, textarea, select {
        width: 100%;
        padding: 11px;
        margin-top: 4px;
        border-radius: 6px;
        border: 1px solid #cbd5e1;
        background: #f9fbfc;
        font-size: 15px;
        box-sizing: border-box;
    }

    textarea {
        resize: vertical;
        min-height: 70px;
    }

    button {
        margin-top: 18px;
        width: 100%;
        padding: 13px;
        background: #154360;
        color: white;
        border: none;
        border-radius: 6px;
        font-size: 18px;
        cursor: pointer;
        font-weight: 600;
    }

    button:hover {
        background: #0d2c42;
    }

    .small-btn {
        width: 49%;
        display: inline-block;
    }

    .reset-btn {
        background: #a93226;
    }

    .reset-btn:hover {
        background: #7b241c;
    }

    .copy-btn {
        background: #1e8449;
    }

    .copy-btn:hover {
        background: #145a32;
    }

    #output {
        height: 220px;
        background: #f4f6f9;
        margin-top: 15px;
    }

    .status {
        padding: 10px;
        margin-top: 10px;
        border-radius: 8px;
        font-weight: bold;
    }
    .status-green { background:#d4efdf; color:#145a32; }
    .status-orange { background:#fdebd0; color:#af601a; }
    .status-red { background:#f5b7b1; color:#922b21; }

    ul.checklist { list-style-type:none; padding-left:0; }
    ul.checklist li { margin:6px 0; }
</style>

</head>

<body>

<div class="wrapper">

    <h1>AIDE ETR N1</h1>
    <div class="subtitle">Ouverture de dossier étranger – pour une prise en charge complète et sans erreur.</div>

    <!-- SECTION 1 – Dates & Informations -->
    <h2>1. Informations générales</h2>

    <label>Date d’entrée dans le pays</label>
    <input type="date" id="dateEntree">

    <div class="small-help">Calcul automatique des jours sur place.</div>
    <div id="joursDisplay" class="status"></div>

    <label><input type="checkbox" id="panneRecente"> Le client est tombé en panne récemment (&lt; 30 jours)</label>

    <label>Langues parlées</label>
    <input type="text" id="langues" placeholder="Ex : Français, Anglais">

    <label>Destination finale</label>
    <input type="text" id="destination" placeholder="Ville, hôtel, famille…">

    <label>Évènement (raison de la panne)</label>
    <textarea id="evenement"></textarea>

    <label>Second numéro de téléphone</label>
    <input type="text" id="numero2" placeholder="+34… (numéro local si possible)">

    <label>Notes internes / remarques</label>
    <textarea id="notes"></textarea>


    <!-- SECTION 2 – Localisation -->
    <h2>2. Localisation précise</h2>

    <label>Pays</label>
    <input list="listePays" id="pays" placeholder="Commence à taper pour chercher…">
    <datalist id="listePays">
        <option>France</option>
        <option>Espagne</option>
        <option>Portugal</option>
        <option>Italie</option>
        <option>Allemagne</option>
        <option>Belgique</option>
        <option>Luxembourg</option>
        <option>Suisse</option>
        <option>Pays-Bas</option>
        <option>Autriche</option>
        <option>Royaume-Uni</option>
    </datalist>

    <label>Localisation précise</label>
    <div class="small-help">Si autoroute : PK + direction. Si Espagne : PK indispensable.</div>
    <textarea id="localisation"></textarea>

    <label><input type="checkbox" id="autoroute"> Le client est sur autoroute</label>

    <div id="zoneAutoroute" style="display:none;">
        <label>Numéro autoroute</label>
        <input type="text" id="numAutoroute">

        <label>Point Kilométrique (PK)</label>
        <input type="text" id="pk">

        <label>Direction</label>
        <input type="text" id="direction">
    </div>

    <label>Google Plus Code</label>
    <input type="text" id="pluscode" placeholder="Ex : 87G8+4W Madrid">
    <div id="pluscodeStatus"></div>


    <!-- SECTION 3 – Véhicule -->
    <h2>3. Véhicule</h2>

    <label>Type de véhicule</label>
    <input list="listeVehicules" id="vehicule" placeholder="Commence à taper…">
    <datalist id="listeVehicules">
        <option>Véhicule léger</option>
        <option>Véhicule utilitaire</option>
        <option>Camping-car</option>
        <option>Moto</option>
        <option>Taxi</option>
        <option>Caravane</option>
        <option>Remorque</option>
        <option>Voiture de location</option>
    </datalist>

    <label><input type="checkbox" id="grosVehicule"> Gros véhicule ou spécificités particulières</label>

    <div id="zoneSpecVehicule" style="display:none;">
        <label>Spécificités du véhicule</label>
        <textarea id="specVehicule" placeholder="Dimensions, poids, équipements, porte moto…"></textarea>
    </div>


    <!-- SECTION 4 – Script -->
    <h2>4. Génération du script</h2>

    <button onclick="generateScript()">Générer le script</button>
    <textarea id="output" readonly></textarea>

    <button class="copy-btn" onclick="copyScript()">Copier le script</button>
    <button class="reset-btn" onclick="resetForm()">Réinitialiser</button>


    <!-- SECTION 5 – Checklist -->
    <h2>5. Checklist N1</h2>
    <div id="checklist"></div>

</div>

<script>
// ---------- CALCUL DES JOURS ----------
document.getElementById("dateEntree").addEventListener("change", calculateDays);
document.getElementById("panneRecente").addEventListener("change", calculateDays);

function calculateDays() {
    let dateEntree = document.getElementById("dateEntree").value;
    let display = document.getElementById("joursDisplay");

    if (!dateEntree) {
        display.innerHTML = "";
        return;
    }

    let entree = new Date(dateEntree);
    let now = new Date();
    let diff = Math.floor((now - entree) / (1000*60*60*24));

    let panne = document.getElementById("panneRecente").checked;

    if (diff < 60)
        display.className = "status status-green";
    else if (diff <= 90)
        display.className = "status status-orange";
    else
        display.className = panne ? "status status-orange" : "status status-red";

    let assist = "";
    if (diff > 90 && panne) assist = "✅ Assistable (panne récente)";
    else if (diff > 90) assist = "❌ NON assistable (VHC > 90 jours)";
    else assist = "✅ Assistable";

    display.innerHTML = `Séjour : ${diff} jours — ${assist}`;
}


// ---------- AUTOROUTE ----------
document.getElementById("autoroute").addEventListener("change", () => {
    document.getElementById("zoneAutoroute").style.display =
        document.getElementById("autoroute").checked ? "block" : "none";
});

// ---------- GROS VEHICULE ----------
document.getElementById("grosVehicule").addEventListener("change", () => {
    document.getElementById("zoneSpecVehicule").style.display =
        document.getElementById("grosVehicule").checked ? "block" : "none";
});

// ---------- PLUS CODE ----------
document.getElementById("pluscode").addEventListener("input", () => {
    let pc = document.getElementById("pluscode").value.trim();
    let status = document.getElementById("pluscodeStatus");

    const regex = /^[A-Z0-9]{4}\+[A-Z0-9]{2}$/i;

    if (regex.test(pc)) {
        status.innerHTML = "✅ Plus Code valide";
        status.style.color = "green";
    } else {
        status.innerHTML = "❌ Format invalide (ex : 87G8+4W)";
        status.style.color = "red";
    }
});

// ---------- SCRIPT FINAL ----------
function generateScript() {
    let script =
`📅 Séjour :
Date d'entrée : ${document.getElementById("dateEntree").value}
${document.getElementById("joursDisplay").innerText}

🗣 Langues parlées :
${document.getElementById("langues").value}

🎯 Destination finale :
${document.getElementById("destination").value}

💥 Évènement :
${document.getElementById("evenement").value}

📞 Second numéro :
${document.getElementById("numero2").value}

📍 Localisation précise :
Pays : ${document.getElementById("pays").value}
${document.getElementById("localisation").value}

`;

    if (document.getElementById("autoroute").checked) {
        script += `🛣 Autoroute :
Numéro : ${document.getElementById("numAutoroute").value}
PK : ${document.getElementById("pk").value}
Direction : ${document.getElementById("direction").value}

`;
    }

    if (document.getElementById("pluscode").value.trim() !== "") {
        script += `🌍 Plus Code Google :
${document.getElementById("pluscode").value}
Lien : https://maps.google.com/?q=${encodeURIComponent(document.getElementById("pluscode").value)}

`;
    }

    script += `🚐 Véhicule :
Type : ${document.getElementById("vehicule").value}
`;

    if (document.getElementById("grosVehicule").checked) {
        script += `Spécificités : 
${document.getElementById("specVehicule").value}
`;
    }

    script += `📝 Notes internes :
${document.getElementById("notes").value}
`;

    document.getElementById("output").value = script;

    generateChecklist();
}

// ---------- CHECKLIST ----------
function generateChecklist() {
    let out = "<ul class='checklist'>";

    function check(item, ok) {
        out += `<li>${ok ? "✅" : "❌"} ${item}</li>`;
    }

    check("Date d'entrée renseignée", document.getElementById("dateEntree").value !== "");
    check("Langues renseignées", document.getElementById("langues").value !== "");
    check("Destination renseignée", document.getElementById("destination").value !== "");
    check("Localisation complète", document.getElementById("localisation").value !== "");
    check("Évènement renseigné", document.getElementById("evenement").value !== "");
    check("2e numéro renseigné", document.getElementById("numero2").value !== "");
    check("Type véhicule sélectionné", document.getElementById("vehicule").value !== "");
    check("Plus Code valide si renseigné",
        document.getElementById("pluscode").value === "" ||
        /^[A-Z0-9]{4}\+[A-Z0-9]{2}$/i.test(document.getElementById("pluscode").value)
    );

    out += "</ul>";

    document.getElementById("checklist").innerHTML = out;
}

// ---------- RESET ----------
function resetForm() {
    document.querySelectorAll("input, textarea").forEach(x => x.value = "");
    document.getElementById("autoroute").checked = false;
    document.getElementById("grosVehicule").checked = false;
    document.getElementById("zoneAutoroute").style.display = "none";
    document.getElementById("zoneSpecVehicule").style.display = "none";
    document.getElementById("joursDisplay").innerHTML = "";
    document.getElementById("pluscodeStatus").innerHTML = "";
    document.getElementById("output").value = "";
    document.getElementById("checklist").innerHTML = "";
}
</script>

</body>
</html>
